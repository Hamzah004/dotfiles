...
location_id = "942cbbb9755ee77ac6a824bcae7bb541f57d80241c4c462af10f1060dfbb52a9"  # TODO
...
t_prediction = (
    f"  滛 ({forecast_type}) {t_prediction}" if len(t_prediction) > 0 else t_prediction
)
...
# tooltip text with modern styling
tooltip_text = str.format(
    "<b>🌍 Current Weather</b>\n\n{}\n{}\n<big>{}</big>\n\n<b>📊 Details</b>\n{}\n{}\n{}\n\n<b>📈 Forecast</b>\n{}\n{}",
    f'<span size="xx-large" foreground="#89B4FA">{temp}</span>',
    f'<span size="large">{icon}</span>',
    status,
    f"<small>{temp_feel_text}</small>",
    f"🌡️  {temp_min_max}",
    f"💨 {wind_text}    💧 {humidity_text}",
    f"👁️  {visbility_text}    🌬️ AQI {air_quality_index}",
    f"<i>{r_prediction}</i>",
    f"<i>{t_prediction}</i>",
)
